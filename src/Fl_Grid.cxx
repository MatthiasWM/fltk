...
class Fl_Grid::Row {
  ...
  ~Row() {
    // fprintf(stderr, "Delete Row, Row::free_cells() ...\n");
    free_cells();
  };
  ...
  void remove_cell(int col) { // remove cell of column col from list of cells
    Cell *cel = cells_;
    Cell *prev = 0;
    while (cel) {
      Cell *next = cel->next_;
...
void Fl_Grid::init() {
  ...
  need_layout_ = true;            // layout needs to be calculated
}
...
#if (0)
  fprintf(stderr, "[%3d] layout(): %d x %d, children = %d\n", __LINE__, rows_, cols_, children());
#endif
...
#if (0)
  fprintf(stderr, "[%4d] columns: tcwi=%3d, tcwe=%3d, hcwe=%3d, icwe=%3d\n", __LINE__, tcwi, tcwe, hcwe, icwe);
  fprintf(stderr, "[%4d] rows:    trhe=%3d, trwe=%3d, hrwe=%3d, irwe=%3d\n", __LINE__, trhe, trwe, hrwe, irwe);
#endif
  ...
  if (space > 0 && tcwe > 0) {
    ...
    for (int c = 0; c < cols_; c++, col++) {
      if (col->weight_ > 0) {
        ...
        remaining -= add_space;
#if (0)
        fprintf(stderr, "[%4d] added %3d to col %2d width  (weight = %d, remaining = %4d) => %d\n",
                __LINE__, add_space, c, col->weight_, remaining, col->w_);
#endif
      }
    }
    if (remaining != 0)
  ...
  if (space > 0 && trwe > 0) {
    ...
    for (int r = 0; r < rows_; r++, row++) {
      if (row->weight_ > 0) {
        ...
        remaining -= add_space;
#if (0)
        fprintf(stderr, "[%4d] added %3d to row %2d height (weight = %d, remaining = %4d) => %d\n",
                __LINE__, add_space, r, row->weight_, remaining, row->h_);
#endif
      }
    }
    if (remaining != 0)
  ...
  for (int r = 0; r < rows_; r++, row++) {
    ...
    for (int c = 0; c < cols_; c++, col++) {
      ...
      if (cel) {
        ...
        if (wi && wi->visible()) {
#if (0)
          fprintf(stderr, "[%4d] Cell (%2d, %2d), (x0, y0) = (%3d, %3d), w_ = %d, h_ = %d\n",
                  __LINE__, r, c, x0, y0, cel->w_, cel->h_);
#endif
          // calculate the cell's position and size, take spanning into account
...
#if (0)
          fprintf(stderr, "[%4d]   ...  resize(%3d, %3d, %3d, %3d)\n", __LINE__, wx, wy, ww, wh);
#endif
...
#if (0)
  fprintf(stderr, "[%4d] Fl_Grid::on_remove(%p [%s]): child(%d): %p [%s] will be removed (children: %d)\n",
          __LINE__, this, label() ? label() : "?", index, wi, wi->label() ? wi->label() : "?",
          children());
#endif
  Cell *c = cell(wi); // find the cell of this child
  if (c) {
#if (0)
    fprintf(stderr, "Fl_Grid::child %d (%p) has been removed from cell %p\n", index, wi, c);
#endif
...
void Fl_Grid::resize(int X, int Y, int W, int H) {
  ...
  static int rcount = 0;
  rcount++;
  fprintf(stderr, "\n[%4d] resize() from (%3d, %3d, %3d, %3d) to (%3d, %3d, %3d, %3d)\n",
          rcount, old_size.x(), old_size.y(), old_size.w(), old_size.h(),
          X, Y, W, H);
#endif

#if (1)
  need_layout(1); // delay layout calculation until draw() is called
#else
  layout();
#endif
}
...
// set minimal column width and row height and row and column specific gaps and weights
