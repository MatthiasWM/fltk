...
/*
 The fltk3::Widget::type_ property is primarily used as a subtype field to further
...
void fltk3::Widget::default_callback(fltk3::Widget *widget, void * /*v*/) {
...
/**
    ...
    callback \p static fltk3::Widget::default_callback() that puts a pointer
    ...
    overflows, the oldest entry (fltk3::Widget *) is discarded.
    ...
    set one with fltk3::Widget::callback(Fl_Callback *cb, void *data)
    or one of its variants.

    \see fltk3::Widget::callback()
    \see fltk3::Widget::callback(Fl_Callback *cb, void *data)
    \see fltk3::Widget::default_callback()
*/
fltk3::Widget *Fl::readqueue() {
  if (obj_tail==obj_head) return 0;
  fltk3::Widget *widget = obj_queue[obj_tail++];
  if (obj_tail >= QUEUE_SIZE) obj_tail = 0;
  return widget;
}
/*
...
/**
  ...
  This means that fltk3::Widget::draw_focus() or one of the more specialized
  ...
  \see fltk3::Widget::draw_focus()
  \see fltk3::Widget::draw_focus(Fl_Boxtype, int, int, int, int) const
*/
void fltk3::Widget::draw_focus(Fl_Boxtype bt, int X, int Y, int W, int H, Fl_Color bg) const {
...
void fltk3::Widget::label(const char *a) {
...
void fltk3::Widget::copy_label(const char *a) {
  // reassigning a copied label remains the same copied label
...
void fltk3::Widget::image(Fl_Image* img) {
  if (image_bound()) {
...
void fltk3::Widget::bind_image(Fl_Image* img) {
  image(img);
  bind_image( (img != NULL) );
}

void fltk3::Widget::deimage(Fl_Image* img) {
  if (deimage_bound()) {
...
void fltk3::Widget::bind_deimage(Fl_Image* img) {
  deimage(img);
 ...
 */
void fltk3::Widget::do_callback(fltk3::Widget *widget, void *arg, Fl_Callback_Reason reason) {
 ...
 */
void fltk3::Widget::user_data(void* v) {
  if ((flags_ & AUTO_DELETE_USER_DATA) && user_data_)
    delete (Fl_Callback_User_Data*)user_data_;
 ...
 */
void fltk3::Widget::user_data(Fl_Callback_User_Data* v, bool auto_free) {
  user_data((void*)v);
  if (auto_free)
