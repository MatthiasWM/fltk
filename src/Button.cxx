...
#include <fltk3/Fl_Radio_Button.H>
#include <fltk3/Fl_Toggle_Button.H>
...
int fltk3::Button::handle(int event) {
  if (shim_)
    return static_cast<IWidgetShim*>(shim_)->handle(event);
  else
    return handle_impl_(event);
}
int fltk3::Button::handle_impl_(int event) {
  int newval;
  switch (event) {
    ...
    case fltk3::KEYBOARD:
      ...
      triggered_by_keyboard: // from fltk3::SHORTCUT
        if (type() == FL_RADIO_BUTTON) {
          if (!value_) {
            setonly();
            set_changed();
...
/**
 ...
 changed. It's called during `fltk3::RELEASE` and `fltk3::KEYBOARD` events with
 `FL_REASON_RELEASED` set as the callback reason.

 Setting `FL_WHEN_CHANGED` will call the callback with `FL_REASON_CHANGED`
 every time the value of the button changes during `fltk3::DRAG`, `fltk3::RELEASE`,
 and `fltk3::KEYBOARD` events.

 Setting `FL_WHEN_NOT_CHANGED` will trigger a callback during `fltk3::RELEASE`
 events, even if the value of the button die *not* change. For radio buttons,
 this is also true during `fltk3::KEYBOARD` events.
 ...
 */
void fltk3::Button::compact(uchar v) { compact_ = v; }

/// (for backwards compatibility)
void fltk3::Button::shortcut(const char *s) {shortcut(fl_old_shortcut(s));}
