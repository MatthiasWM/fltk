...
int Fd_Code_Writer::write_code(const char *s, const char *t, bool to_sourceview) {
  ...
  const char *hdr = "// generated by Fast Light User Interface Designer (fluid) version %.4f\n\n";
  fprintf(header_file, hdr, FL_VERSION);
  fprintf(code_file, hdr, FL_VERSION);
//  line_tag(4, 0, Fl_Preferences::new_UUID()); // FIXME: we don;t need those anymore, right?
//  line_tag(5, 0, Fl_Preferences::new_UUID());
//  fprintf(code_file, "\n");
...
static int crc16(unsigned char *d, long n) {
...
// FIXME: use CRC-32 from zlib
// FIXME: do we want the text extension?
// FIXME: some types need no UID
// FIXME: all types must have a symbolic name
// FIXME: end tha source file with a TAG
void Fd_Code_Writer::line_tag(int kind, unsigned short uid, const char *text)
{
  unsigned short crc = 0xFFFF;
  size_t tag_end = ftell(code_file);
  fseek(code_file, tag_start, SEEK_SET);
  for (long i=tag_start; i<tag_end; i++) {
    unsigned char c = fgetc(code_file);
    crc ^= (unsigned short)c;
    for (int i = 8; i != 0; i--) {
      if ((crc & 0x001) != 0) {
        crc >>= 1;
  ...
  fprintf(code_file, "//~~%d~%04x~%04x~~%s\n", kind, (int)uid, (int)crc, text ? text : "");
  tag_start = ftell(code_file);
}

extern Fl_Window *the_panel;

// FIXME: set up an undo operation
// FIXME: trap errors and let the user decide what to do (dialog box)
// Code block states are:
//   static block:
//     changed by external editor (code file CRC is wrong) -> .fl and .cxx are out of syn, or user modified static block
//   editable block:
//     changed by external editor (code file CRC is wrong) -> not an error, user changed code externally, read result back
//     changed in FLUID (code CRC doesn't match FLUID CRC) -> not an error, user changed code in FLUID, ignore
//     changed in FLUID and code file -> conflict! give user the choice to abort or choose a side
// FIXME: add level of mergeback support to user settings
// FIXME: add mergeback support to project settings
// FIXME: command line option for mergeback
// FIXME: manual mergeback always give us a result dialog box
// FIXME: automatic mergeback when a new project is loaded and a dialog box suggesting action
// FIXME: automatic mergeback when FLUID becomes app in focus
// FIXME: automatic mergeback on timer when file changes?
int Fd_Code_Writer::merge_back(const char *s, const char *t) {
//  write_code("/Users/matt/dev/fltk-1.4.matt/test.cxx", "/Users/matt/dev/fltk-1.4.matt/test.h");
//  FILE *live = fopen("/Users/matt/dev/fltk-1.4.matt/test.cxx");
  bool changed = false;
  FILE *code = fopen(s, "rb");
  int line_no = 0;
  for (;;) {
    char line[1024];
    fgets(line, 1023, code);
    if (feof(code)) break;
    line_no++;
    const char *tag = strstr(line, "//~~");
    if (tag) {
      // (must return 3) sscanf(tag, "//~~%d~%04x~%04x~~", kind(==0), uid(==irrelevant), crc(==important))
      long tag_end, tag_start = ftell(code);
      for (;;) {
        tag_end = ftell(code);
        fgets(line, 1023, code); 
        if (feof(code)) break;
        ...
        tag = strstr(line, "//~~");
        if (tag) {
          // FIXME: don't see TAG blocks as something special, just treat the last read block according to `kind`
          int kind, uid, crc;
          if (sscanf(tag, "//~~%d~%04x~%04x~~", &kind, &uid, &crc)!=3) {
            fprintf(stderr, "Tag malformed: %s\n", tag);
            continue;
          }
          printf("Customizable block kind %d found at line %d, uid %d, %ld bytes\n", kind, line_no, uid, tag_end-tag_start);
          long n = tag_end-tag_start;
          if (n==0) { } // set string to empty!
          unsigned char *cblock = (unsigned char*)malloc(n+1);
          long curr = ftell(code);
          fseek(code, tag_start, SEEK_SET);
          fread(cblock, 1, n, code);
          cblock[n] = 0;
          printf("  cblock: %ld bytes read\n", n);
          int cblock_crc = crc16(cblock, n);
          printf("  cblock: was %schanged.\n", cblock_crc==crc ? "NOT ": "");
          if (cblock_crc!=crc) {
            if (kind==2) { // callback
              Fl_Type *tp = Fl_Type::find_by_uid(uid);
              if (!tp) {
                printf("No widget with UID %d found!\n", uid);
              }
              if (tp && tp->is_a(Fl_Type::ID_Widget_)) {
                Fl_Widget_Type *wt = (Fl_Widget_Type*)tp;
                wt->callback((char*)cblock); // FIXME: unindent this!
                changed = true;
              }
            }
          }
          fseek(code, curr, SEEK_SET);
          free(cblock);
          break;
        }
      }
