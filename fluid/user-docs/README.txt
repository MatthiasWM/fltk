...
To read the docs after downloading or generating them, open the files:
    ...
    documentation/fluid-user/html/index.html
    documentation/fluid-user.pdf
...
Documentation can be found at these URL's:
    ...
    HTML: https://fltk.gitlab.io/fltk/fluid-user
    PDF:  https://fltk.gitlab.io/fltk/fluid-user.pdf


Generation of Documentation with autoconf (configure) + make:
-------------------------------------------------------------
Use "make fluid_user_html" in the documentation directory to generate the HTML
documentation, or "make fluid_user_pdf" to generate the PDF version.
"make fluid_user_docs" generates both versions in one step (as would do
"make clean fluid_user_html fluid_user_docs").
...
automatically (it is excluded from the default target "ALL").
  ...
  - OPTION_BUILD_FLUID_USER_HTML_DOCS:BOOL=ON     target: fluid_user_html
  - OPTION_BUILD_FLUID_USER_PDF_DOCS:BOOL=ON      target: fluid_user_pdf
...
and the corresponding build options are ON.
  ...
  - OPTION_INSTALL_FLUID_USER_HTML_DOCS:BOOL=OFF
  - OPTION_INSTALL_FLUID_USER_PDF_DOCS:BOOL=OFF
